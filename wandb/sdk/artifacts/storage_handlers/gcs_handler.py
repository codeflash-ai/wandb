...
def _handle_gcp_import_error(exc: ImportError) -> Never:
...
class GCSHandler(StorageHandler):
    ...
    def load_path(
        ...
        # Try, in order:
        obj = (
            # First attempt to get the generation, if specified.
            ...
            or
            # Object versioning is disabled on the bucket, or the version ID wasn't available,
    ...
    def store_path(
        ...
        # and key.
        gcs_path = _ParsedGCSUri.from_uri(path)
        ...
        start_time = None
        bucket_obj = client.bucket(gcs_path.bucket)
        obj = bucket_obj.get_blob(gcs_path.key, generation=gcs_path.version)
        ...
        if multi := ((obj is None) or obj.name.endswith("/")):
            ...
            termlog(
                f'Generating checksum for up to {max_objects} objects with prefix "{gcs_path.key}"... ',
                newline=False,
            )
            objects = bucket_obj.list_blobs(
    ...
    def _entry_from_obj(
        ...
        """
        uri = _ParsedGCSUri.from_uri(path)
